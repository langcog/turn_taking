...
In another study, \citet*{torreiraUndRev} used a combination of button-press and verbal responses to investigate the relationship between lexicosyntactic and prosodic cues in turn-boundary identification. They found that participants never verbally responded or pressed the ``turn-end'' button when hearing a syntactically-complete unit without an intonational phrase boundary. However, utterance-level prosodic boundaries embedded in multi-utterance turns could trick participants into anticipating a turn end 29\% of the time (see also de \citet{de-ruiter2006}:525). Their findings corroborate other corpus and experimental findings promoting a combination of cues (lexicosyntactic, prosodic, and pragmatic) as key for accurate turn-end prediction \citep{duncan1972, ford1996, hirvenkari2013}.

In sum, adults accurately and spontaneously make predictions about upcoming turn structure. They use a wide variety of cues in making their predictions and then act immediately upon them. Their predictive processing relies on a sophisticated body of knowledge about linguistic structure, non-verbal signals, and social actions. The later acquisition of verbal turn-taking skills must then be tied to children's knowledge about language, gaze, gesture, and social cues. But children's turn taking starts early in infancy, long before first words or gestures emerge; a primary role for lexicosyntactic cues doesn't fit well with children's pre-verbal turn-taking. 
...
% add bit about children's prosodic development and link this to the plans for the experiments
% mention that we will be looking at anticipatory gaze switching
...
% check over again to make as clear as possible
...
\end{figure}
% evidence for integration of diff cues?
...
We tested these findings in two linear mixed-effects models of participants' baseline-corrected anticipatory switches---one model each for children and adults. The child model included condition (\textit{normal}, \textit{words only}, \textit{prosody only}, and \textit{no speech}), transition type (question vs. non-question), age, and gap duration as fixed effects, with full interactions between condition, transition type, and age. The model also included random effects of item (turn transition) and participant, with random slopes of condition and transition type for participants. The adult model matched the child model, excluding age-related effects.
...
% no main effects is kind of meaningless
...
% potentially makes more sense w/ pros intro
...
% mention adult counter evidence again, elaborate on prosody a little (and add refs after "knowing..." sentence)
...
% mention again that our prosody manipulation included durational changes
...
%include subheadings to look at diffs w/ adult vs. chi literature
...
% move to the front of this section
