...
\begin{abstract}
We investigate language acquisition through the lens of a fundamental conversational skill: turn taking. Children begin developing turn-taking skills in infancy, but take several years to assimilate language into their turn-taking systems. In two eye-tracking experiments with children ages 1--6, we measured participants' spontaneous predictions about upcoming speaker change while controlling the amount of linguistic information available. We found that children already predicted upcoming turns at age one, and integrated linguistic cues differently at different ages. Children under 3;0 showed a general advantage for prosody over lexicosyntax, contrary to the typical finding for adults. Children 2;0 and older showed more predictive switches for questions than non-questions, but only when lexical information was available. But, we found no evidence that lexicosyntax is sufficient for turn prediction---participants' performance was best overall with access to lexicosyntax and prosody together. Our results point to the importance of interaction in children’s real-time linguistic processing.
\end{abstract}
...
Although the Keitel et al. \citeyearpar{keitel2013} study constitutes a substantial advance over previous work, it nevertheless has several limitations. Because these limitations directly inform our own study design, we review them in some detail. First, their estimates of baseline ``random'' gaze frequency were computed from switches that occurred while the speaker's turn was ongoing (i.e, during turns, and not during inter-turn gaps). Their estimates of anticipatory shifting frequency were computed from switches that occurred within the time window immediately surrounding inter-turn gaps (i.e., around inter-turn gaps, and primarily not during turns). Assuming that gaze switches happen most often during the inter-turn gap \citep{hirvenkari2013}, switches made during ongoing speech (their ``random'' baseline frequency) would be at a minimum. Therefore their ``random'' (baseline) frequency actually maximized the possibility of finding a difference between ``random'' and anticipatory gaze frequencies. A more conservative baseline would be to compare participants' looking behavior at turn transitions to their looking behavior during randomly-selected windows of time throughout the stimulus. We follow this latter approach in our work. 
...
Thanks! (To be filled in, but must include NSF GRFP, ERC, Bing, and the CDM).
