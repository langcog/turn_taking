...
read.data <- function(path,file.name) {
    ...
    data <- within(data, {
        ...
        F.looks <- L.Object.Hit == "FEMALE" | R.Object.Hit == "FEMALE"
        M.looks <- L.Object.Hit == "MALE" | R.Object.Hit == "MALE"
        ...
        Version <- rep(name.parts[2], nrow(data))
        AgeGroup <- rep(name.parts[1], nrow(data))
        Subject <- rep(paste(name.parts[1], name.parts[2], name.parts[3],
            sep = "-"), nrow(data))
