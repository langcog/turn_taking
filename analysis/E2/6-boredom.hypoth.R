...
fixations <- data.table(findFixations(all.data))
...
realdata <- data.frame(Time=numeric(),SecBin=character(),
	Switches=numeric(),SwitchesSE=numeric())
for (j in 1:length(unique(boredom.fixes$SecBins))) {
	switches <- rep(0, length(time))
	switchesSE <- rep(0, length(time))
	...
	for (k in 1:length(time)) {
		switchedaway <- nrow(subset(subdata, LookAway < time[k]))/nswitches
		switchesSE[k] <- sqrt((switchedaway*(1 - switchedaway))/nswitches)
		switches[k] <- 1 - switchedaway
	}
	newswitches <- data.frame(Time=time,SecBin=secbins,
		Switches=switches,SwitchesSE=switchesSE)
	realdata <- rbind(realdata, newswitches)
}
realdata$Sample <- "Actual data"
...
boredlooksSE <- rep(0, length(boredlooks))
# Fill it into the cumulative data frame for plotting
boredom <- data.frame(Time=numeric(),SecBin=character(),
	Switches=numeric(),SwitchesSE=numeric())
	for (j in 1:length(unique(boredom.fixes$SecBins))) {
		secbin <- as.character(unique(boredom.fixes$SecBins)[j])
		secbins <- rep(secbin,length(time))
		newswitches <- data.frame(Time=time,SecBin=secbins,
			Switches=boredlooks,SwitchesSE=boredlooksSE)
		boredom <- rbind(boredom, newswitches)
	}
boredom$Sample <- "Boredom-driven"
...
limits <- aes(ymax = switches + switchesSE, ymin = switches - switchesSE)
dodge <- position_dodge(width=0.9)

# Plot and save the results
boredom.hyp <- qplot(Time, Switches, group = Sample,
	ylim = c(0,1), ymin = Switches - SwitchesSE,
	ymax = Switches + SwitchesSE, color=Sample, 
	xlab = "Time (sec)", ylab = "Proportion gazing at the speaker\n",
	position=position_dodge(width=.1), data=rvb.clipped) +
    ...
    legend.justification = c(1,0), legend.position = c(0.88,0.07),
